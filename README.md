...
- Supabase Postgres for persistence (with in-memory fallback for local dev).
...
Open http://localhost:3000. No Supabase required — an in-memory store kicks in when `SUPABASE_URL` is not set. The data evaporates when the dev server stops, which is what you want for testing.
...
3. **Create a Supabase project** at https://supabase.com/dashboard (or pick an existing one).
4. **Run the schema** — in Supabase → SQL Editor → New query → paste the contents of `supabase-schema.sql` from this repo → Run. This creates the `round_config` and `players` tables and enables RLS.
5. **Grab the Supabase credentials**:
   - `SUPABASE_URL` — Supabase → Project Settings → Data API → "Project URL"
   - `SUPABASE_SERVICE_ROLE_KEY` — Supabase → Project Settings → API → "service_role" key (NOT the anon key — the service role bypasses RLS, which is what the server needs)
6. **Set env vars in Vercel** → Project → Settings → Environment Variables:
   - `SUPABASE_URL`
   - `SUPABASE_SERVICE_ROLE_KEY`
   - `ADMIN_KEY` — a long random string. Generate with:
     ```bash
     node -e "console.log(require('crypto').randomBytes(24).toString('base64url'))"
     ```
7. **Redeploy** so the env vars take effect. The production URL (e.g. `https://stableford-app.vercel.app`) is what you share with players.
8. **Bookmark the admin URL**: `https://<your-domain>/admin?key=<ADMIN_KEY>`
...
- [ ] Deploy to Vercel with Supabase env vars + `ADMIN_KEY` set.
...
lib/
  ...
  store.ts              CRUD on top of Supabase, in-memory fallback for local dev
...
supabase-schema.sql     one-time schema for round_config + players tables
tests/
